...
export default async function Home(){
  ...
  return <div className={styles.page}>
    ...
    <main><HeroSlider/>
      <section className={styles.features} aria-labelledby="explore-title"><div className={styles.sectionIntro}><span>EXPLORE TIV SONGS</span><h2 id="explore-title">Music, people and living heritage.</h2></div><div className={styles.featureGrid}>{features.map(feature=><Link className={styles.feature} href={feature.href} key={feature.label}><span className={styles.featureImage}><Image src={feature.image} alt="" fill sizes="(max-width: 600px) 42vw, 180px"/></span><strong>{feature.label}</strong><p>{feature.description}</p></Link>)}</div></section>
