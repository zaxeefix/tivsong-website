...
export default function HeroSlider(){
  const [slides,setSlides]=useState<Slide[]>([fallback]),[active,setActive]=useState(0);
  const load=useCallback(()=>fetch("/api/cms/entries/hero",{cache:"no-store"}).then(response=>response.ok?response.json():[]).then((items:Slide[])=>{setSlides(items.length?items:[fallback]);setActive(0)}).catch(()=>setSlides([fallback])),[]);
  ...
  useEffect(()=>{if(slides.length<2)return;const timer=window.setInterval(()=>setActive(value=>(value+1)%slides.length),7000);return()=>window.clearInterval(timer)},[slides.length]);
