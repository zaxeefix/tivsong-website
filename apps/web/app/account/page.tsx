...
type Account={id:string;email:string;username:string;displayName:string;accountType:"individual"|"artist";status:string;roles:string[];artist:{stageName:string;verifiedAt:string|null}|null};
...
function uploadWithProgress<T>(data:FormData,onProgress:(percent:number,stage:string)=>void):Promise<T>{
  return new Promise((resolve,reject)=>{
    ...
    xhr.onload=()=>{
      ...
      if(xhr.status>=200&&xhr.status<300){onProgress(100,"Complete");resolve(body)}
      else reject(new Error(body?.error||`Upload failed (${xhr.status})`));
    };
    xhr.onerror=()=>reject(new Error("Upload connection failed. Check your internet connection and try again."));
    xhr.ontimeout=()=>reject(new Error("The upload exceeded three minutes. Try a smaller file or faster connection."));
    xhr.send(data);
  });
}
...
export default function AccountPage(){
  ...
  const load=useCallback(async()=>{
    try{
      ...
      setAccount(me);setSongs(items);setVideos(videoItems);setCategories(categoryItems);setView("dashboard");setError("");
    }catch(reason){
  ...
  useEffect(()=>{if(sessionStorage.getItem("tiv-account-auth"))void load();else if(new URLSearchParams(window.location.search).get("mode")==="register")setView("register")},[load]);
  ...
  const submit=async(event:FormEvent<HTMLFormElement>,kind:"login"|"register"|"media")=>{
    ...
    try{
      if(kind==="login"){
        ...
        const result=await request<{message:string}>("/account/register",{method:"POST",body:JSON.stringify({...payload,accountType})});
  ...
  if(isRegisterView(view))return <main className={styles.page}>
    ...
    <section className={styles.shell}><div className={styles.intro}><span>JOIN TIV SONGS</span><h1>Create the right account.</h1><p>Member accounts are for listening and community participation. Artist accounts create a professional profile and require administrator approval before uploading music.</p><div className={styles.promises}><div><strong>Accurate</strong><small>Contact and location information helps protect your account.</small></div><div><strong>Private</strong><small>Your phone and account details are not displayed publicly.</small></div><div><strong>Verified</strong><small>Artist applications are reviewed before publication.</small></div></div></div>
      <section className={styles.card}><div className={styles.cardHead}><div><span>CREATE ACCOUNT</span><h2>{accountType==="artist"?"Artist application":"Member registration"}</h2></div><button className={styles.textButton} onClick={()=>{setView("login");setError("");setMessage("")}}>Sign in</button></div>{error&&<p className={styles.error}>{error}</p>}{message&&<p className={styles.success}>{message}</p>}
        ...
        <form className={`${styles.form} ${styles.formGrid}`} onSubmit={event=>submit(event,"register")}><label>Full name<input name="displayName" autoComplete="name" required minLength={2} maxLength={100}/></label><label>Username<input name="username" autoComplete="username" required minLength={3} maxLength={40} pattern="[A-Za-z0-9_]+"/></label><label>Email address<input name="email" type="email" autoComplete="email" required/></label><label>Phone number<input name="phoneNumber" type="tel" autoComplete="tel" required minLength={7} maxLength={30} placeholder="e.g. +234 800 000 0000"/></label><label>Country<input name="country" autoComplete="country-name" required minLength={2} maxLength={100} placeholder="e.g. Nigeria"/></label><label>State / province<input name="state" autoComplete="address-level1" required maxLength={120} placeholder="e.g. Benue"/></label><label className={styles.full}>Password<input name="password" type="password" autoComplete="new-password" minLength={8} maxLength={100} required/></label>{accountType==="artist"&&<><label>Stage name<input name="stageName" required minLength={2} maxLength={100}/></label><label className={styles.full}>Professional biography<textarea name="bio" rows={5} maxLength={2000} placeholder="Experience, releases, performances and cultural contribution" required/></label><p className={`${styles.notice} ${styles.full}`}>Your application will appear in the administrator's artist approval queue. Upload tools unlock only after approval.</p></>}<button className={`${styles.primary} ${styles.full}`} disabled={busy}>{busy?"Creating account…":accountType==="artist"?"Submit artist application":"Create member account"}</button></form>
      </section>
    </section><SiteFooter variant="account"/>
  </main>;

  if(view==="dashboard"&&account&&!account.artist)return <main className={styles.page}>
    <nav className={styles.nav}><Link className={styles.brand} href="/"><Image src="/assets/tiv-song-logo.jpeg" alt="" width={48} height={48}/>TIV SONGS</Link><div className={styles.navActions}><PrimaryNavLinks linkClassName={styles.navLink} activeClassName={styles.activeNav}/><ThemeToggle className={styles.theme}/><button className={styles.navLink} onClick={logout}>Log out</button></div></nav>
    <section className={styles.shell}><div className={styles.dashboard}><header className={styles.dashboardHead}><div><span>MEMBER ACCOUNT</span><h1>Welcome, {account.displayName}</h1><p>{account.email}</p></div><div className={`${styles.badge} ${styles.approved}`}>Account active</div></header>{error&&<p className={styles.error}>{error}</p>}{message&&<p className={styles.success}>{message}</p>}<div className={styles.dashboardGrid}><section className={styles.card}><span>YOUR TIV SONGS ACCOUNT</span><h2>Listen, discover and participate.</h2><p>Your member account is separate from artist profiles. You can follow Tiv culture, join respectful discussions, report harmful content and share community activities.</p><div className={styles.accountTypes}><Link className={styles.selected} href="/"><strong>Discover music</strong><small>Explore artists, songs and videos</small></Link><Link href="/community"><strong>Community</strong><small>Join activities and conversations</small></Link></div></section><section className={styles.card}><span>ARTIST ACCOUNTS</span><h2>Want to publish music?</h2><p>Music and video submission tools are available only to registered artist accounts. This member account will never be listed as an artist.</p><p className={styles.notice}>To become an artist, create a dedicated artist account with a unique email and stage name for administrator verification.</p></section></div></div></section>
